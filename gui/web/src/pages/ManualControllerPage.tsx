...
const stateForMode: Record<DriveMode, string> = {
    mow: "MANUAL_MOWING",
    transport: "MANUAL_DRIVING",
    record: "RECORDING",
};

const zeroTwist = (): TwistStamped => ({
...
export const ManualControllerPage = () => {
    ...
    const sendZero = useCallback(() => {
        ...
        joyStream.sendJsonMessage(zeroTwist());
    }, [joyStream.sendJsonMessage]);

    useEffect(() => {
        clearTimeout(armTimerRef.current);
        if (!requestedState) return;
        if (stateName === requestedState) {
            // The status subscriber and the serial bridge receive the same
            // BT update independently. Wait for two 2 Hz bridge periods so
            // the STM32 has received the new blade-safe mode before cmd_vel
            // is allowed through this page.
            armTimerRef.current = setTimeout(() => setArmed(true), ARM_SETTLE_MS);
            return () => clearTimeout(armTimerRef.current);
        }
        setArmed(false);
        sendZero();
        return () => clearTimeout(armTimerRef.current);
    }, [requestedState, sendZero, stateName]);
    ...
    const run = async (nextMode: DriveMode) => {
        ...
        setMode(nextMode);
        setRequestedState(stateForMode[nextMode]);
    };
    const stop = async () => {
        sendZero();
        setArmed(false);
        setRequestedState(undefined);
    ...
    return (
        <section aria-label={t("manualController.ariaLabel")} style={{width: "100%", minWidth: 0, maxWidth: 1180, margin: "0 auto"}}>
            ...
            <div style={{display: "grid", gridTemplateColumns: dualStick ? "minmax(0, 0.85fr) minmax(0, 1.2fr) minmax(0, 0.85fr)" : "minmax(0, 0.9fr) minmax(0, 1.1fr)", gap: 16, alignItems: "stretch"}} className={`manual-controller-grid ${dualStick ? "manual-controller-dual" : "manual-controller-combined"}`}>
                {dualStick ? <JoystickPanel label={t("manualController.driveStick")} hint={t("manualController.driveHint")} compact={compactLandscape} disabled={!armed}>
                ...
                </JoystickPanel> : <JoystickPanel label={t("manualController.combinedStick")} hint={t("manualController.combinedHint")} combined disabled={!armed}>
                ...
                {dualStick && <JoystickPanel label={t("manualController.steerStick")} hint={t("manualController.steerHint")} compact={compactLandscape} disabled={!armed}>
...
const JoystickPanel = ({label, hint, children, combined = false, compact = false, disabled = false}: {label: string; hint: string; children: React.ReactNode; combined?: boolean; compact?: boolean; disabled?: boolean}) => {
    const {colors} = useThemeMode();
    return <div style={{display: "grid", minWidth: 0, placeItems: "center", gap: compact ? 6 : 10, padding: compact ? 8 : combined ? 20 : 16, textAlign: "center", borderRadius: 20, background: colors.panel, border: `1px solid ${colors.border}`}}>
        ...
        <div style={{touchAction: "none", opacity: disabled ? 0.45 : 1, pointerEvents: disabled ? "none" : undefined}}>{children}</div>
