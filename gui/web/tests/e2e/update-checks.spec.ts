...
for (const mobile of [false,true]) {
    test(`read-only checks distinguish catalog and registry on ${mobile ? 'mobile' : 'desktop'}`, async ({page}) => {
        ...
        await page.route('**/api/system/updates?*', route => {
            ...
            return route.fulfill({json: {
                channel, state: checking ? channel === 'dev' ? 'unverified' : 'available' : 'not_checked',
                source: channel === 'dev' ? 'registry' : 'catalog',
                version: checking ? channel === 'dev' ? 'dev' : 'v1.2.0' : undefined,
                ...
                components: checking ? [{name:'mowgli-gui',state:'changed',installed_revision:'aaaaaaa',available_revision:'bbbbbbb',custom_image:true,digest_reference:true,available_image:'ghcr.io/mowglinext/mowglinext/mowglinext-gui@sha256:'+'b'.repeat(64)}] : [],
            }});
        });
        ...
        await expect(card.getByText('Individual images checked — bundle unverified', {exact:true})).toBeVisible();
        ...
        await expect(card.getByText('Not checked yet', {exact:true})).toBeVisible();
        await card.getByRole('button', {name:'Check now'}).click();
        await expect(card.getByText('A different published deployment is available', {exact:true})).toBeVisible();
        ...
        await expect(card.getByText('A different published deployment is available', {exact:true})).toHaveCount(0);
