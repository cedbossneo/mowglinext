...
TEST(Perf, BareTickThroughput) {
  ...
  for (int i = 0; i < kN; ++i) {
    ...
    double dt = TimeMillis([&] { gm.Tick(now_s); });
  ...
  const double last100_avg = std::accumulate(
      per_tick_ms.end() - 100, per_tick_ms.end(), 0.0) / 100.0;
  std::printf(
      "[Perf] BareTickThroughput N=%d  median=%.2f ms  p95=%.2f ms  "
      "last100_avg=%.2f ms\n",
      kN, med, p95, last100_avg);
...
TEST(Perf, MowingSessionWithScansAndLC) {
  ...
  for (int i = 0; i < kN; ++i) {
    ...
    double dt = TimeMillis([&] {
      ...
      if (out) {
        ...
        // the warmup.
        if (i > 200 && i % 50 == 0) {
          auto cands = gm.FindLoopClosureCandidates(
              out->node_index, 5.0, 5.0, 1);
          if (!cands.empty()) {
            auto cand_scan = gm.GetScan(cands[0]);
            auto cand_pose = gm.GetPose(cands[0]);
            if (!cand_scan.empty() && cand_pose) {
              auto res = matcher.Match(
                  cand_scan, all_scans[i],
                  cand_pose->between(out->pose));
              if (res.ok) {
                gm.AddLoopClosure(cands[0], out->node_index,
                                  res.delta, 0.05, 0.02);
              }
            }
          }
  ...
  std::printf(
      ...
      kN, (unsigned long)stats.total_nodes,
      (unsigned long)stats.scans_attached,
      (unsigned long)stats.loop_closures, med, p95, last100_avg);
