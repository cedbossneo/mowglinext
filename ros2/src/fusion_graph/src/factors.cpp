...
GnssLeverArmFactor::GnssLeverArmFactor(
    gtsam::Key key,
...
YawUnaryFactor::YawUnaryFactor(
    gtsam::Key key, double meas_yaw,
    const gtsam::SharedNoiseModel& model)
    : Base(model, key),
      meas_yaw_(meas_yaw) {}

gtsam::Vector YawUnaryFactor::evaluateError(
    const gtsam::Pose2& X,
    OptMat H) const {
  gtsam::Vector1 e;
