...
| Submodule pins | `.gitmodules` (`universal-gnss` → `mowglinext` fork, branch `fix/rover-dynamic-mode-uav`; `opennav_coverage` → upstream `main`) |
...
- `ros2/src/external/universal-gnss/` — `mowglinext/universal-gnss` fork submodule @ `4bf5d0a2` (branch `fix/rover-dynamic-mode-uav`, issue #395 fixes stacked); revert `.gitmodules` to `pepeuch/universal-gnss` once upstreamed. Top level: `gnss_core/ gnss_driver/ gnss_ntrip/ gnss_protocols/ gnss_ros2/ gnss_tools/ gnss_transport/ docs/ examples/ testdata/ MOWGLINEXT_TODO.md`.
