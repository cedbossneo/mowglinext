...
void updateOwner(OwnerState &owner, const diagnostic_msgs::msg::DiagnosticArray &diagnostics,
...
bool isFresh(const std::optional<Snapshot> &snapshot, CorrectionDiagnosticTracker::TimePoint now,
...
class CorrectionDiagnosticTracker::Impl {
 public:
  ...
  void apply(PublicGnssStatus &status, TimePoint now) const {
    ...
    if (current_ntrip != nullptr && status.correction_transport_status ==
...
void CorrectionDiagnosticTracker::update(const diagnostic_msgs::msg::DiagnosticArray &diagnostics,
