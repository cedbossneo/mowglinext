...
Status makeStatus(const char *name, const char *source, const char *message = "",
                  Values values = {}) {
  Status status;
  status.name = name;
...
Array healthyNtrip(const char *source = "caster:2101/MOUNT", std::int32_t stamp = 10) {
  ...
  diagnostics.status = {
      makeStatus("universal_gnss_ntrip/summary", source, "healthy",
      ...
      makeStatus("universal_gnss_ntrip/rtcm_semantic/base_station_arp", source, "valid",
                 {{"seen", "true"},
                  {"decoded", "true"},
                  {"valid", "true"},
                  {"message_type", "1006"},
                  {"station_id", "42"},
                  {"age_s", "20.0"}}),
      makeStatus("universal_gnss_ntrip/rtcm_semantic/msm_summary", source, "valid",
                 {{"seen", "true"},
                  {"decoded", "true"},
                  {"valid", "true"},
                  {"message_type", "1077"},
...
Array receiverSnapshot(const char *message, std::int32_t stamp = 20) {
  ...
  diagnostics.status = {
      ...
      makeStatus("universal_gnss/rtcm_semantic/msm_summary", kSource, "valid",
                 {{"seen", "true"},
                  {"decoded", "true"},
                  {"valid", "true"},
                  {"message_type", "1087"},
...
PublicGnssStatus project(const CorrectionDiagnosticTracker &tracker,
...
TEST(CorrectionDiagnosticTrackerTest, HealthyDynamicFlowDoesNotRequireGlonass1230) {
  ...
  EXPECT_EQ(status.correction_transport_status,
...
TEST(CorrectionDiagnosticTrackerTest, ConnectionResponseAndForwardingDoNotImplySemanticHealth) {
  ...
  accepted_array.status = {
      ...
      makeStatus("universal_gnss_ntrip/rtcm_forwarding", "caster:2101/MOUNT",
...
TEST(CorrectionDiagnosticTrackerTest, MalformedZeroCellAndStaticOnlyCorrectionsFailClosed) {
  ...
  EXPECT_EQ(project(malformed, now).correction_semantic_status,
  ...
  EXPECT_EQ(project(zero_cell, now).correction_semantic_status,
            PublicGnssStatus::CORRECTION_SEMANTIC_STATUS_INVALID);

  auto static_only_array = healthyNtrip();
  static_only_array.status.erase(
      std::remove_if(static_only_array.status.begin(), static_only_array.status.end(),
                     [](const Status &entry) {
  ...
  EXPECT_EQ(project(static_only, now).correction_semantic_status,
            PublicGnssStatus::CORRECTION_SEMANTIC_STATUS_WAITING);
}
...
TEST(CorrectionDiagnosticTrackerTest, DisconnectReconnectSourceAndStationChangesDoNotInherit) {
  ...
  EXPECT_EQ(project(tracker, now + std::chrono::milliseconds(400)).correction_semantic_status,
            PublicGnssStatus::CORRECTION_SEMANTIC_STATUS_INVALID);
}
...
TEST(CorrectionDiagnosticTrackerTest, OmitClearOlderSetAndLaterCurrentSetRemainDistinct) {
  ...
  EXPECT_EQ(project(tracker, now + std::chrono::milliseconds(50)).value_flags &
  ...
  EXPECT_EQ(project(tracker, now + std::chrono::milliseconds(100)).correction_semantic_status,
            PublicGnssStatus::CORRECTION_SEMANTIC_STATUS_HEALTHY);
  ...
  EXPECT_EQ(project(tracker, now + std::chrono::milliseconds(200)).correction_semantic_status,
            PublicGnssStatus::CORRECTION_SEMANTIC_STATUS_UNAVAILABLE);
