...
- **universal-gnss is a submodule on the mowglinext FORK**, branch `fix/rover-dynamic-mode-uav` (`.gitmodules`, issue #395: GLONASS-1230 + UM980 `MODE ROVER UAV`). A bump means re-pinning the gitlink, not just editing the branch line; with the submodule uninitialised the Dockerfile's seven `COPY`s have nothing to copy and the build fails.
