...
1. **robot_localization (dual EKF) is the sole localizer.** The stack runs two cooperating EKFs under `two_d_mode` (roll, pitch and Z are clamped to zero — it is effectively a 15D filter with only x, y, yaw, vx, vy, vyaw actively fused). `ekf_odom_node` fuses wheel twist (`/wheel_odom`, with a tight `vy≈0` covariance that enforces non-holonomic motion without a custom pseudo-measurement) + IMU gyro (`/imu/data`, roll/pitch + gyro_z) and publishes `odom → base_footprint`. `navsat_to_absolute_pose_node` converts `/gps/fix` to `/gps/pose_cov` (PoseWithCovarianceStamped in map frame, lever-arm-corrected) using a fixed datum from `mowgli_robot.yaml` (so saved area polygons and dock pose survive restarts). `ekf_map_node` fuses the same wheel+IMU inputs plus `/gps/pose_cov` (`pose0`), `/imu/cog_heading` (absolute yaw derived from GPS course-over-ground by `cog_to_imu.py`), `/imu/mag_yaw` (tilt-compensated magnetometer yaw from `mag_yaw_publisher.py`, when calibrated), and `/slam/pose_cov` (`pose1`, from the slam_toolbox RTK fallback below) and publishes `map → odom`. RTK-Fixed σ ~3 mm flows through as-is because the EKF honors `NavSatFix.covariance` directly. When LiDAR is present, **slam_toolbox** runs as an RTK fallback: it publishes `map_slam → wheel_odom_raw` on a **fully decoupled parallel TF tree** (`wheel_odom_raw → base_footprint_wheels → lidar_link_wheels`) so none of its inputs depend on the fused state. `wheel_odom_tf_node` integrates raw `/wheel_odom` into the parallel tree's motion prior; `slam_scan_frame_relay` mirrors the URDF sensor extrinsic onto the parallel tree and republishes `/scan` on `/scan_slam`. `slam_pose_anchor_node` continuously EWMA-aligns `map → map_slam` to the GPS-anchored map frame (gated on `NavSatFix.status == GBAS_FIX` AND ekf cov_xx < 1e-2) and republishes the slam pose into the gps map frame as `/slam/pose_cov` with covariance that grows as time-since-last-RTK-Fixed grows. The EKF therefore weights GPS above slam when RTK is healthy (σ ~3 mm vs floor 10 cm) and slam above GPS during RTK-Float / dropout windows, naturally and without hard gating. slam_toolbox runs in `lifelong` mode against a persisted map at `/ros2_ws/maps/slam.{posegraph,data}` if present, else `mapping` mode (first session); `slam_map_persist_node` checkpoints the graph on every transition out of `HIGH_LEVEL_STATE_RECORDING` so the map is built during area recording and refined across subsequent sessions.
...
- **slam_toolbox RTK fallback:** Gated on `use_lidar`. slam_toolbox runs on a parallel TF tree (`wheel_odom_raw → base_footprint_wheels → lidar_link_wheels`) that is fully decoupled from the fused state, so its motion prior can never depend on its own earlier output. Three mowgli_localization helper nodes make this work: `wheel_odom_tf_node` integrates raw `/wheel_odom` twist into the parallel motion-prior TF, `slam_scan_frame_relay` mirrors the URDF sensor extrinsic onto the parallel tree and republishes `/scan` as `/scan_slam` with the matching frame_id, and `slam_pose_anchor_node` continuously EWMA-aligns `map → map_slam` to the GPS-anchored map frame on every RTK-Fixed sample (gated on `NavSatFix.status == GBAS_FIX` AND ekf cov_xx < 1e-2) and republishes the slam pose into the gps map frame as `/slam/pose_cov` with covariance that grows as time-since-last-RTK-Fixed grows (floor 10 cm, drift 1 cm/s, capped 5 m). `ekf_map_node` fuses this as `pose1` (x, y only — yaw covariance is held high so cog/mag yaw dominates). With RTK-Fixed (σ ~3 mm), GPS dominates naturally; with RTK-Float (σ ~30 cm) slam competes; with no fix slam carries the map-frame estimate. slam_toolbox runs in `lifelong` mode against `/ros2_ws/maps/slam.{posegraph,data}` if present, else `mapping`. `slam_map_persist_node` checkpoints the graph on every transition out of `HIGH_LEVEL_STATE_RECORDING` (3) so the map is built during area recording and refined across subsequent sessions. The `/map` OccupancyGrid that Nav2 reads is still published by `mowgli_map/map_server_node` from user-defined area polygons — slam_toolbox's occupancy is not surfaced to Nav2.
...
- Do NOT publish a `map→odom` TF from slam_toolbox, Nav2, or any other node. `map→odom` is owned by `ekf_map_node`. slam_toolbox publishes `map_slam→wheel_odom_raw` on the parallel tree, and `slam_pose_anchor_node` publishes `map→map_slam`. The slam pose enters the EKF only as `/slam/pose_cov` (`ekf_map_node.pose1`), never as a TF on the fused tree.
- Do NOT feed slam_toolbox or any LiDAR-derived pose into the parallel tree's motion prior — slam_toolbox reads `wheel_odom_raw → base_footprint_wheels` (raw wheel odometry only) and the mirrored sensor frame `lidar_link_wheels` via `/scan_slam`. Pointing its `odom_frame` at `odom` or its `base_frame` at `base_footprint` would create a feedback loop through the EKF.
- Do NOT re-introduce Kinematic-ICP. It was replaced by slam_toolbox: K-ICP gave only a body-frame twist (no absolute-pose information), which couldn't carry the map-frame estimate during multi-minute RTK-Float windows. The submodule under `ros2/src/kinematic_icp/` is retained for revertability but the launch + helper scripts are gone.
