...
vi.mock('@mowglinext/common', async () => {
    const actual = await vi.importActual('@mowglinext/common');
    return {
        ...actual,
        AsyncButton: ({children, onAsyncClick, ...props}: any) => (
            <button onClick={onAsyncClick} {...props}>{children}</button>
        ),
        AsyncDropDownButton: ({children, ...props}: any) => (
